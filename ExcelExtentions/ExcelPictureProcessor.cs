    ...
    internal class ExcelPictureProcessor {
        public static void SetPictureToCell(string picPath, ExcelWorksheet workSheet, int row, int col, int magicToken) {
            using (Bitmap img = new Bitmap(picPath)) {
                set(img, workSheet, row, col, magicToken);
            }
        }
        public static void SetPictureToCell(Stream stream, ExcelWorksheet workSheet, int row, int col, int magicToken) {
            using (Bitmap img = new Bitmap(stream)) {
                set(img, workSheet, row, col, magicToken);
            }
        }
        private static void set(Bitmap img, ExcelWorksheet workSheet, int row, int col, int magicToken) {
